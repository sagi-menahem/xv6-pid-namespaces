// File: proc.c	Name:Sagi Menahem	ID:208645937
#include "types.h"
#include "defs.h"
#include "param.h"
#include "memlayout.h"
...
myproc(void) {
 ...
 // cps137 - Prints details of all processes in the system.
 //
 // This function iterates over the process table and prints process details,
 // including name, PID, state, external PID, parent PID, and CPU time.
 // It ensures safe access to the process table using a lock.
int cps137(void) {
    struct proc *p;
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if (p->state == UNUSED) // Skip unused process entries
            continue;

        // Get the external PID (extpid) based on the namespace
        int extpid = get_pid_for_ns(p, initproc->nsproxy->pid_ns);

        // Get the parent PID (ppid) in the current namespace
        int ppid = 0;
        if (p->parent) {
            // Retrieve the parent PID from the parent's namespace
            ppid = get_pid_for_ns(p->parent, p->parent->nsproxy->pid_ns);
            if (ppid == 0) {
                // If no valid parent PID, default to the init process PID
                ppid = get_pid_for_ns(initproc, initproc->nsproxy->pid_ns);
            }
        }

        // Determine the state of the process
        const char *state = (p->state == RUNNING) ? "RUNNING" : "SLEEPING";

        // Print the process details: name, PID, state, extpid, ppid, and CPU time
        cprintf("%s \t %d \t %s \t %d \t\t %d \t %d \n",
                p->name, p->ns_pid, state, extpid, ppid, p->cpu_time);
    }

    // Unlock the process table after iterating
    release(&ptable.lock);
